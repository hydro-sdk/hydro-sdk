...
void main() {
  ...
  group("", () {
    test("", () async {
      ...
      final componentSearchResult = await api.searchComponents(
        searchTerm: componentName,
      );

      final componentSearchSuccessResult = componentSearchResult.maybeWhen(
        success: (val) => val,
        orElse: () => null,
      );

      expect(componentSearchSuccessResult, isNotNull);
      expect(componentSearchSuccessResult!.result.isNotEmpty, true);
      expect(componentSearchSuccessResult.result.length, 1);

      final componentSearchResult2 = await api.searchComponents(
        searchTerm: "test",
      );

      final componentSearchSuccessResult2 = componentSearchResult2.maybeWhen(
        success: (val) => val,
        orElse: () => null,
      );

      expect(componentSearchSuccessResult2, isNotNull);
      expect(componentSearchSuccessResult2!.result.isNotEmpty, true);
      expect(componentSearchSuccessResult2.result.length, greaterThan(1));
      expect(
          componentSearchSuccessResult2.result.length, lessThanOrEqualTo(10));
