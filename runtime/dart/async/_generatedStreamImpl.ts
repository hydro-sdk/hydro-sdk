...
import { I_StreamImpl } from "./_streamImpl";
...
declare const dart: {
    async: {
        _generatedStreamImpl: <T>(
            this: void,
            _generatedStreamImpl: I_GeneratedStreamImpl<T>,
            _pending: () => I_PendingEvents<T>
        ) => I_GeneratedStreamImpl<T>;
    };
};
export interface I_GeneratedStreamImpl<T> {
    listen: (
        onData?: (data: T) => void,
        props?: {
...
export class _GeneratedStreamImpl<T> implements I_StreamImpl<T> {
    public constructor(_pending: () => I_PendingEvents<T>) {
        dart.async._generatedStreamImpl(this, _pending);
    }
