...
declare const dart: {
    async: {
        _takeStream: <T>(
            this: void,
            _takeStream: I_TakeStream<T>,
            source: IStream<T>,
            count: number
        ) => I_TakeStream<T>;
    };
};
export interface I_TakeStream<T> {
    listen: (
        onData?: (value: T) => void,
        props?: {
...
export class _TakeStream<T> implements I_ForwardingStream<T, T> {
    public constructor(source: IStream<T>, count: number) {
        dart.async._takeStream(this, source, count);
    }
