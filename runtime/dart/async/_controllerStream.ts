...
declare const dart: {
    async: {
        _controllerStream: <T>(
            this: void,
            _controllerStream: I_ControllerStream<T>,
            _controller: I_StreamControllerLifecycle<T>
        ) => I_ControllerStream<T>;
    };
};
export interface I_ControllerStream<T> {
    getHashCode: () => number;
    listen: (
        onData?: (data: T) => void,
        props?: {
...
export class _ControllerStream<T> implements I_StreamImpl<T> {
    public constructor(_controller: I_StreamControllerLifecycle<T>) {
        dart.async._controllerStream(this, _controller);
    }
