...
declare const dart: {
    async: {
        _whereStream: <T>(
            this: void,
            _whereStream: I_WhereStream<T>,
            source: IStream<T>,
            test: (value: T) => boolean
        ) => I_WhereStream<T>;
    };
};
export interface I_WhereStream<T> {
    listen: (
        onData?: (value: T) => void,
        props?: {
...
export class _WhereStream<T> implements I_ForwardingStream<T, T> {
    public constructor(source: IStream<T>, test: (value: T) => boolean) {
        dart.async._whereStream(this, source, test);
    }
