...
declare const dart: {
    async: {
        _forwardingStream: <S, T>(
            this: void,
            _forwardingStream: I_ForwardingStream<S, T>,
            _source: IStream<S>
        ) => I_ForwardingStream<S, T>;
    };
};
export interface I_ForwardingStream<S, T> {
...
export class _ForwardingStream<S, T> implements IStream<T> {
    public constructor(_source: IStream<S>) {
        dart.async._forwardingStream(this, _source);
    }
