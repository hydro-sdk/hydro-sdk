...
declare const dart: {
    async: {
        _expandStream: <S, T>(
            this: void,
            _expandStream: I_ExpandStream<S, T>,
            source: IStream<S>,
            expand: (event: S) => IIterable<T>
        ) => I_ExpandStream<S, T>;
    };
};
export interface I_ExpandStream<S, T> {
    listen: (
        onData?: (value: T) => void,
        props?: {
...
export class _ExpandStream<S, T> implements I_ForwardingStream<S, T> {
    public constructor(source: IStream<S>, expand: (event: S) => IIterable<T>) {
        dart.async._expandStream(this, source, expand);
    }
