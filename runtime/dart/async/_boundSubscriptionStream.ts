...
declare const dart: {
    async: {
        _boundSubscriptionStream: <S, T>(
            this: void,
            _boundSubscriptionStream: I_BoundSubscriptionStream<S, T>,
            _stream: IStream<S>,
            _onListen: (
                stream: IStream<S>,
                cancelOnError: boolean
            ) => IStreamSubscription<T>
        ) => I_BoundSubscriptionStream<S, T>;
    };
};
export interface I_BoundSubscriptionStream<S, T> {
...
export class _BoundSubscriptionStream<S, T> implements IStream<T> {
    public constructor(
        _stream: IStream<S>,
        _onListen: (
            stream: IStream<S>,
        ...
        dart.async._boundSubscriptionStream(this, _stream, _onListen);
    }
