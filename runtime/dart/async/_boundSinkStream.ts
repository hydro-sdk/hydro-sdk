...
declare const dart: {
    async: {
        _boundSinkStream: <S, T>(
            this: void,
            _boundSinkStream: I_BoundSinkStream<S, T>,
            _stream: IStream<S>,
            _sinkMapper: (output: IEventSink<T>) => IEventSink<S>
        ) => I_BoundSinkStream<S, T>;
    };
};
export interface I_BoundSinkStream<S, T> {
...
export class _BoundSinkStream<S, T> implements IStream<T> {
    public constructor(
        _stream: IStream<S>,
        _sinkMapper: (output: IEventSink<T>) => IEventSink<S>
    ) {
        dart.async._boundSinkStream(this, _stream, _sinkMapper);
    }
