...
import { I_ControllerStream } from "./_controllerStream";
...
declare const dart: {
    async: {
        _broadcastStream: <T>(
            this: void,
            _broadcastStream: I_BroadcastStream<T>,
            controller: I_StreamControllerLifecycle<T>
        ) => I_BroadcastStream<T>;
    };
};
export interface I_BroadcastStream<T> {
...
export class _BroadcastStream<T> implements I_ControllerStream<T> {
    public constructor(controller: I_StreamControllerLifecycle<T>) {
        dart.async._broadcastStream(this, controller);
    }
