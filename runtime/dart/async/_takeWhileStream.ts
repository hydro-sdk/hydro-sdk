...
declare const dart: {
    async: {
        _takeWhileStream: <T>(
            this: void,
            _takeWhileStream: I_TakeWhileStream<T>,
            source: IStream<T>,
            test: (value: T) => boolean
        ) => I_TakeWhileStream<T>;
    };
};
export interface I_TakeWhileStream<T> {
    listen: (
        onData?: (value: T) => void,
        props?: {
...
export class _TakeWhileStream<T> implements I_ForwardingStream<T, T> {
    public constructor(source: IStream<T>, test: (value: T) => boolean) {
        dart.async._takeWhileStream(this, source, test);
    }
