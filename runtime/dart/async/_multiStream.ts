...
declare const dart: {
    async: {
        _multiStream: <T>(
            this: void,
            _multiStream: I_MultiStream<T>,
            _onListen: (__: IMultiStreamController<T>) => void,
            isBroadcast: boolean
        ) => I_MultiStream<T>;
    };
};
export interface I_MultiStream<T> {
    isBroadcast: boolean;
    listen: (
        onData?: (event: T) => void,
        props?: {
...
export class _MultiStream<T> implements IStream<T> {
    public readonly isBroadcast: boolean = undefined as any;
    public constructor(
        _onListen: (__: IMultiStreamController<T>) => void,
        isBroadcast: boolean
    ) {
        dart.async._multiStream(this, _onListen, isBroadcast);
    }
