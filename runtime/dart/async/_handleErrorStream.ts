...
declare const dart: {
    async: {
        _handleErrorStream: <T>(
            this: void,
            _handleErrorStream: I_HandleErrorStream<T>,
            source: IStream<T>,
            onError: IFunction,
            test?: (error: Object) => boolean
        ) => I_HandleErrorStream<T>;
    };
};
export interface I_HandleErrorStream<T> {
    listen: (
        onData?: (value: T) => void,
        props?: {
...
export class _HandleErrorStream<T> implements I_ForwardingStream<T, T> {
    public constructor(
        source: IStream<T>,
        onError: IFunction,
        test?: (error: Object) => boolean
    ) {
        dart.async._handleErrorStream(this, source, onError, test);
    }
