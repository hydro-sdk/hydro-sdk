...
declare const dart: {
    async: {
        _skipWhileStream: <T>(
            this: void,
            _skipWhileStream: I_SkipWhileStream<T>,
            source: IStream<T>,
            test: (value: T) => boolean
        ) => I_SkipWhileStream<T>;
    };
};
export interface I_SkipWhileStream<T> {
    listen: (
        onData?: (value: T) => void,
        props?: {
...
export class _SkipWhileStream<T> implements I_ForwardingStream<T, T> {
    public constructor(source: IStream<T>, test: (value: T) => boolean) {
        dart.async._skipWhileStream(this, source, test);
    }
