...
declare const dart: {
    async: {
        _skipStream: <T>(
            this: void,
            _skipStream: I_SkipStream<T>,
            source: IStream<T>,
            count: number
        ) => I_SkipStream<T>;
    };
};
export interface I_SkipStream<T> {
    listen: (
        onData?: (value: T) => void,
        props?: {
...
export class _SkipStream<T> implements I_ForwardingStream<T, T> {
    public constructor(source: IStream<T>, count: number) {
        dart.async._skipStream(this, source, count);
    }
