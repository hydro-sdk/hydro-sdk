...
declare const dart: {
    collection: {
        _splayTreeValueIterable: <K, V>(
            this: void,
            _splayTreeValueIterable: I_SplayTreeValueIterable<K, V>,
            _map: ISplayTreeMap<K, V>
        ) => I_SplayTreeValueIterable<K, V>;
    };
};
export interface I_SplayTreeValueIterable<K, V> {
    getLength: () => number;
    getIsEmpty: () => boolean;
...
export class _SplayTreeValueIterable<K, V>
    implements IEfficientLengthIterable<V>
{
    public constructor(_map: ISplayTreeMap<K, V>) {
        dart.collection._splayTreeValueIterable(this, _map);
    }
