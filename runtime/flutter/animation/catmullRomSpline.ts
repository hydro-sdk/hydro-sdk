...
declare const flutter: {
    animation: {
        catmullRomSpline: (
            this: void,
            catmullRomSpline: ICatmullRomSpline,
            controlPoints: IList<IOffset>,
            props: {
                endHandle?: IOffset | undefined;
                startHandle?: IOffset | undefined;
                tension: number;
            }
        ) => ICatmullRomSpline;
    };
...
export interface ICatmullRomSpline {
    ...
    generateSamples: (props: {
        end: number;
        start: number;
        tolerance: number;
    }) => IIterable<ICurve2DSample>;
    findInverse: (x: number) => number;
    transform: (t: number) => IOffset;
    toString: () => string;
    getHashCode: () => number;
}
export class CatmullRomSpline implements ICurve2D {
    public constructor(
        controlPoints: IList<IOffset>,
        props: {
            endHandle?: IOffset | undefined;
    ...
    private readonly _dart_transformInternal: (t: number) => IOffset =
        undefined as any;
    private readonly _dart_generateSamples: (props: {
        end: number;
        start: number;
        tolerance: number;
    }) => IIterable<ICurve2DSample> = undefined as any;
    private readonly _dart_findInverse: (x: number) => number =
        undefined as any;
