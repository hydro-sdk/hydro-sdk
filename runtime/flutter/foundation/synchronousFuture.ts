...
declare const flutter: {
    foundation: {
        synchronousFuture: <T>(
            this: void,
            synchronousFuture: ISynchronousFuture<T>,
            _value: T
        ) => ISynchronousFuture<T>;
    };
};
export interface ISynchronousFuture<T> {
    asStream: () => IStream<T>;
    catchError: (
        onError: IFunction,
...
export class SynchronousFuture<T> implements IFuture<T> {
