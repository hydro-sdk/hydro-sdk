...
declare const flutter: {
    foundation: {
        hashedObserverList: <T>(
            this: void,
            hashedObserverList: IHashedObserverList<T>
        ) => IHashedObserverList<T>;
    };
};
export interface IHashedObserverList<T> {
    add: (item: T) => void;
...
export class HashedObserverList<T> implements IIterable<T> {
    public constructor() {
    ...
    private readonly _dart_followedBy: (other: IIterable<T>) => IIterable<T> =
        undefined as any;
    private readonly _dart_map: <T>(toElement: (e: T) => T) => IIterable<T> =
        undefined as any;
    ...
    private readonly _dart_expand: <T>(
        toElements: (element: T) => IIterable<T>
    ) => IIterable<T> = undefined as any;
    private readonly _dart_forEach: (action: (element: T) => void) => void =
        undefined as any;
    private readonly _dart_reduce: (combine: (value: T, element: T) => T) => T =
        undefined as any;
    private readonly _dart_fold: <T>(
        initialValue: T,
        combine: (previousValue: T, element: T) => T
    ) => T = undefined as any;
    private readonly _dart_every: (test: (element: T) => boolean) => boolean =
        undefined as any;
    private readonly _dart_join: (separator: string) => string =
        undefined as any;
    private readonly _dart_any: (test: (element: T) => boolean) => boolean =
        undefined as any;
    private readonly _dart_toList: (props: { growable: boolean }) => IList<T> =
        undefined as any;
    private readonly _dart_toSet: () => ISet<T> = undefined as any;
    private readonly _dart_take: (count: number) => IIterable<T> =
        undefined as any;
    private readonly _dart_takeWhile: (
        test: (value: T) => boolean
    ) => IIterable<T> = undefined as any;
    private readonly _dart_skip: (count: number) => IIterable<T> =
        undefined as any;
