...
declare const flutter: {
    rendering: {
        annotationResult: <T>(
            this: void,
            annotationResult: IAnnotationResult<T>
        ) => IAnnotationResult<T>;
    };
};
export interface IAnnotationResult<T> {
    add: (entry: IAnnotationEntry<T>) => void;
    getEntries: () => IIterable<IAnnotationEntry<T>>;
    getAnnotations: () => IIterable<T>;
}
export class AnnotationResult<T> {
    public constructor() {
        flutter.rendering.annotationResult(this);
    }
    private readonly _dart_add: (entry: IAnnotationEntry<T>) => void =
        undefined as any;
    private readonly _dart_getEntries: () => IIterable<IAnnotationEntry<T>> =
        undefined as any;
    private readonly _dart_getAnnotations: () => IIterable<T> =
        undefined as any;
