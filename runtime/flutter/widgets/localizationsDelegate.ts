...
declare const flutter: {
    widgets: {
        localizationsDelegate: <T>(
            this: void,
            localizationsDelegate: ILocalizationsDelegate<T>
        ) => ILocalizationsDelegate<T>;
    };
};
export interface ILocalizationsDelegate<T> {
...
export class LocalizationsDelegate<T> {
    ...
    private readonly _dart_isSupported: (locale: ILocale) => boolean =
        undefined as any;
    private readonly _dart_load: (locale: ILocale) => IFuture<T> =
        undefined as any;
