...
declare const flutter: {
    physics: {
        _underdampedSolution: (
            this: void,
            _underdampedSolution: I_UnderdampedSolution,
            spring: ISpringDescription,
            distance: number,
            velocity: number
        ) => I_UnderdampedSolution;
    };
};
export interface I_UnderdampedSolution {
    x: (time: number) => number;
    dx: (time: number) => number;
    getType: () => SpringType;
}
export class _UnderdampedSolution implements I_SpringSolution {
    public constructor(
        spring: ISpringDescription,
        distance: number,
        velocity: number
    ) {
        flutter.physics._underdampedSolution(this, spring, distance, velocity);
    }
