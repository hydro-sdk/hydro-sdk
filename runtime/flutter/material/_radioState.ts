...
declare const flutter: {
    material: {
        _radioState: <T>(
            this: void,
            _radioState: I_RadioState<T>
        ) => I_RadioState<T>;
    };
};
export interface I_RadioState<T> {
...
export class _RadioState<T>
    implements
        Omit<
            ...
            | "initState"
        >,
        ITickerProviderStateMixin<IRadio<T>>,
        IToggleableStateMixin<IRadio<T>>,
        IDiagnosticable,
        ITickerProvider
{
    public constructor() {
        flutter.material._radioState(this);
    }
    ...
    private readonly _dart_getWidget: () => IRadio<T> = undefined as any;
