...
export async function addOriginalMappings(
    debugInfo: Array<ModuleDebugInfo>,
    src: TranspiledFile
): Promise<void> {
    await srcMap.SourceMapConsumer.with(
        JSON.parse(src.sourceMap!),
        null,
        (
            consumer:
                | srcMap.BasicSourceMapConsumer
                | srcMap.IndexedSourceMapConsumer
        ) => {
            debugInfo.forEach((x) => {
