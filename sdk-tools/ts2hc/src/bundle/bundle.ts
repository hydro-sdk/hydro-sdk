...
export function bundle(bundleInfo: BundleInfo, logMgr: LogMgr): BundleResult {
    ...
    let entry = Object.values(bundleInfo.entries).find(
        (x) => x.originalFileName == bundleInfo.entry
    );

    if (entry) {
        let bundleEntries = Object.values(bundleInfo.entries).filter(
            (x) => x.originalFileName != bundleInfo.entry
        );
        ...
        bundleEntries.forEach((x) => {
            bundleLines.push(
                `package.preload["${x.moduleName}"] = (function (...)`
            );
        ...
        mainChunkSymbols = mainChunkSymbols
            ...
            .filter(
                (x, i, a) =>
                    a.findIndex(
                        (k) =>
                            k.lineStart == x.lineStart && x.lineEnd == k.lineEnd
                    ) == i
            );

        mangleSymbols(mainChunkSymbols, (symbol: Readonly<ModuleDebugInfo>) => {
