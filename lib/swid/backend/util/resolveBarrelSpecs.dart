...
List<BarrelMember> _extractMembers({
  required final BarrelSpec spec,
}) =>
    ((
      final List<List<BarrelMember>> members,
    ) =>
        members.isNotEmpty
            ? members.reduce((
                value,
      ...
      spec.members
          .map(
            (x) => x.when(
              ...
              fromBarrelSpec: (val) => _extractMembers(
                spec: val,
              ),
            ),
          )
              ...
              ? [
                  ...
                  BarrelSpec(
                    ...
                    name: transformPackageUri(
                      packageUri: element.originalPackagePath,
                    ).split(path.separator).isNotEmpty
                        ? transformPackageUri(
                            packageUri: element.originalPackagePath,
                          )
                            .split(
...
BarrelSpec resolveBarrelSpecs({
  ...
  return _groupMembers(
    ...
    (
      ...
      members: [
        ..._groupMembers(
          members: _extractMembers(
            spec: value,
          ),
        ).map(
          (x) => BarrelMember.fromBarrelSpec(
            barrelSpec: x,
          ),
        ),
