...
class VMManagedHitTestEntry extends _36c2.VMManagedBox<_baac.HitTestEntry> {VMManagedHitTestEntry({required this.table, required this.vmObject, required this.hydroState}) : super(
          table: table,
          vmObject: vmObject,
...
class RTManagedHitTestEntry extends _baac.HitTestEntry implements _36c2.Box<_baac.HitTestEntry> {RTManagedHitTestEntry(_baac.HitTestTarget target$, {required this.table, required this.hydroState}) : super(target$,) { table  [
'vmObject'
] = vmObject;
table  [
'unwrap'
...
void loadHitTestEntry({required _36c2.HydroState hydroState, required _36c2.HydroTable table}) { table  [
'hitTestEntry'
] = _36c2.makeLuaDartFunc(func: (_fac9.List<_fac9.dynamic> luaCallerArguments) { 
return  [RTManagedHitTestEntry(_36c2.maybeUnBoxAndBuildArgument<_baac.HitTestTarget, _fac9.dynamic>(luaCallerArguments  [
1
], parentState: hydroState), table: luaCallerArguments  [
0
], hydroState: hydroState)]; } );
_36c2.registerBoxer<_baac.HitTestEntry>(boxer: ({required _baac.HitTestEntry vmObject, required _36c2.HydroState hydroState, required _36c2.HydroTable table}) { return  VMManagedHitTestEntry(vmObject: vmObject, hydroState: hydroState, table: table); } ); } 
