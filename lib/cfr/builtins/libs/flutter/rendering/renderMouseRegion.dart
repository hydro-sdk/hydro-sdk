...
class VMManagedRenderMouseRegion extends VMManagedBox<RenderMouseRegion> {
  VMManagedRenderMouseRegion(
    ...
    table['setCursor'] =
        makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
      vmObject.cursor = (maybeUnBoxAndBuildArgument<MouseCursor, dynamic>(
          luaCallerArguments[1],
          parentState: hydroState));
      return [];
    });
  ...
  final RenderMouseRegion vmObject;
}

class RTManagedRenderMouseRegion extends RenderMouseRegion
    implements Box<RenderMouseRegion> {
  RTManagedRenderMouseRegion(
      {RenderBox? child,
    ...
    table['cursor'] = maybeBoxObject<MouseCursor>(
        object: this.cursor, hydroState: hydroState, table: HydroTable());
    table['validForMouseTracker'] = this.validForMouseTracker;
    ...
    table['_dart_setOpaque'] =
        makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
      super.opaque = (luaCallerArguments[1]);
      return [];
    });
    ...
    table['_dart_setCursor'] =
        makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
      super.cursor = (maybeUnBoxAndBuildArgument<MouseCursor, dynamic>(
          luaCallerArguments[1],
          parentState: hydroState));
      return [];
    });
  ...
  RenderMouseRegion unwrap() => this;
  RenderMouseRegion get vmObject => this;
  @override
  ...
  @override
  bool get opaque {
    Closure closure = table["getOpaque"];
    return closure.dispatch([table], parentState: hydroState)[0];
  }

  @override
  void set opaque(bool value) {
    Closure closure = table["setOpaque"];
    return closure.dispatch([table], parentState: hydroState)[0];
  }
  ...
  @override
  void set cursor(MouseCursor value) {
    Closure closure = table["setCursor"];
    return closure.dispatch([table], parentState: hydroState)[0];
  }
...
void loadRenderMouseRegion(
    {required HydroState hydroState, required HydroTable table}) {
  table['renderMouseRegion'] =
      makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
    Closure? unpackedonEnter = luaCallerArguments.length >= 2
        ? luaCallerArguments[1]['onEnter']
        : null;
    Closure? unpackedonExit =
        luaCallerArguments.length >= 2 ? luaCallerArguments[1]['onExit'] : null;
    Closure? unpackedonHover = luaCallerArguments.length >= 2
      ...
      RTManagedRenderMouseRegion(
          ...
          child: maybeUnBoxAndBuildArgument<RenderBox?, dynamic>(
              luaCallerArguments.length >= 2
                  ...
                  : null,
              parentState: hydroState),
          cursor: maybeUnBoxAndBuildArgument<MouseCursor, dynamic>(
              luaCallerArguments.length >= 2
          ...
          onHover: unpackedonHover != null
              ? (event) => unpackedonHover.dispatch(
                    [luaCallerArguments[0], event],
                    parentState: hydroState,
                  )
              : null,
  ...
  registerBoxer<RenderMouseRegion>(boxer: (
      {required RenderMouseRegion vmObject,
      required HydroState hydroState,
      required HydroTable table}) {
    return VMManagedRenderMouseRegion(
