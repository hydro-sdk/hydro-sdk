...
class VMManagedSliverHitTestResult extends _36c2.VMManagedBox<_021d.SliverHitTestResult> {VMManagedSliverHitTestResult({required this.table, required this.vmObject, required this.hydroState}) : super(
          table: table,
          vmObject: vmObject,
...
return [vmObject.addWithAxisOffset(crossAxisOffset: luaCallerArguments.length >= 2 ? luaCallerArguments  [
1
]  [
'crossAxisOffset'
] : null?.toDouble(), crossAxisPosition: luaCallerArguments.length >= 2 ? luaCallerArguments  [
1
]  [
'crossAxisPosition'
] : null?.toDouble(), hitTest: (result, {required crossAxisPosition, required mainAxisPosition}) => unpackedhitTest.dispatch([luaCallerArguments[0],result,_36c2.HydroTable.fromMap({"crossAxisPosition":crossAxisPosition,"mainAxisPosition":mainAxisPosition}),],parentState:hydroState,)[0], mainAxisOffset: luaCallerArguments.length >= 2 ? luaCallerArguments  [
1
]  [
'mainAxisOffset'
] : null?.toDouble(), mainAxisPosition: luaCallerArguments.length >= 2 ? luaCallerArguments  [
1
]  [
'mainAxisPosition'
] : null?.toDouble(), paintOffset: _36c2.maybeUnBoxAndBuildArgument<_a643.Offset?, _fac9.dynamic>(luaCallerArguments.length >= 2 ? luaCallerArguments  [
1
]  [
'paintOffset'
...
class RTManagedSliverHitTestResult extends _021d.SliverHitTestResult implements _36c2.Box<_021d.SliverHitTestResult> {RTManagedSliverHitTestResult({required this.table, required this.hydroState}) : super() { table  [
'vmObject'
] = vmObject;
table  [
'unwrap'
...
@_fac9.override _fac9.bool addWithAxisOffset({required _fac9.double crossAxisOffset, required _fac9.double crossAxisPosition, required _fac9.bool Function(_021d.SliverHitTestResult result,{required _fac9.double crossAxisPosition, required _fac9.double mainAxisPosition}) hitTest, required _fac9.double mainAxisOffset, required _fac9.double mainAxisPosition, _a643.Offset? paintOffset}) { 
...
void loadSliverHitTestResult({required _36c2.HydroState hydroState, required _36c2.HydroTable table}) { table  [
'sliverHitTestResult'
] = _36c2.makeLuaDartFunc(func: (_fac9.List<_fac9.dynamic> luaCallerArguments) { 
return  [RTManagedSliverHitTestResult(table: luaCallerArguments  [
0
], hydroState: hydroState)]; } );
_36c2.registerBoxer<_021d.SliverHitTestResult>(boxer: ({required _021d.SliverHitTestResult vmObject, required _36c2.HydroState hydroState, required _36c2.HydroTable table}) { return  VMManagedSliverHitTestResult(vmObject: vmObject, hydroState: hydroState, table: table); } ); } 
