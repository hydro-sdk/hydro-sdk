...
class VMManagedLayer extends VMManagedBox<Layer> {
  VMManagedLayer(
  ...
  final Layer vmObject;
}

class RTManagedLayer extends Layer implements Box<Layer> {
  RTManagedLayer({required this.table, required this.hydroState}) : super() {
    ...
    table['_dart_addToScene'] =
        makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
      addToScene(maybeUnBoxAndBuildArgument<SceneBuilder, dynamic>(
          luaCallerArguments[1],
          parentState: hydroState));
      return [];
    });
...
void loadLayer({required HydroState hydroState, required HydroTable table}) {
  table['layer'] = makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
    return [
      RTManagedLayer(table: luaCallerArguments[0], hydroState: hydroState)
    ];
  });
  registerBoxer<Layer>(boxer: (
      {required Layer vmObject,
