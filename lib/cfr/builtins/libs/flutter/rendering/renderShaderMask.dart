...
class VMManagedRenderShaderMask extends VMManagedBox<RenderShaderMask> {
  VMManagedRenderShaderMask(
    ...
    table['getLayer'] =
      ...
      if (returnValue != null) {
        return [
          maybeBoxObject<ShaderMaskLayer?>(
              object: returnValue, hydroState: hydroState, table: HydroTable()),
        ];
      }
      return [];
    });
    table['getShaderCallback'] =
        makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
      return [
        vmObject.shaderCallback,
      ];
    });
    table['setShaderCallback'] =
        makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
      Closure unpackedvalue = luaCallerArguments[1];
      vmObject.shaderCallback =
          ((bounds) => maybeUnBoxAndBuildArgument<Shader, dynamic>(
  ...
  final RenderShaderMask vmObject;
}

class RTManagedRenderShaderMask extends RenderShaderMask
    implements Box<RenderShaderMask> {
  RTManagedRenderShaderMask(
      {required BlendMode blendMode,
      RenderBox? child,
      required shaderCallback,
      required this.table,
  ...
  RenderShaderMask unwrap() => this;
  RenderShaderMask get vmObject => this;
  @override
  ShaderMaskLayer? get layer {
    Closure closure = table["getLayer"];
    return maybeUnBoxAndBuildArgument<ShaderMaskLayer?, dynamic>(
  ...
  void set shaderCallback(value) {
    Closure closure = table["setShaderCallback"];
...
void loadRenderShaderMask(
  ...
  table['renderShaderMask'] =
      makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
    Closure unpackedshaderCallback = luaCallerArguments.length >= 2
  ...
  registerBoxer<RenderShaderMask>(boxer: (
      {required RenderShaderMask vmObject,
