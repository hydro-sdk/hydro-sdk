...
class VMManagedComponentElement extends VMManagedBox<ComponentElement> {
  VMManagedComponentElement(
    ...
    table['update'] = makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
      vmObject.update(maybeUnBoxAndBuildArgument<Widget, dynamic>(
          luaCallerArguments[1],
          parentState: hydroState));
      return [];
    });
  ...
  final ComponentElement vmObject;
}

class RTManagedComponentElement extends ComponentElement
    implements Box<ComponentElement> {
  RTManagedComponentElement(Widget widget$,
    ...
    table['_dart_build'] =
      ...
      return [
        maybeBoxObject<Widget>(
            object: build(), hydroState: hydroState, table: HydroTable())
      ];
    });
    ...
    table['_dart_update'] =
        makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
      super.update(maybeUnBoxAndBuildArgument<Widget, dynamic>(
          luaCallerArguments[1],
          parentState: hydroState));
      return [];
    });
  ...
  ComponentElement unwrap() => this;
  ComponentElement get vmObject => this;
  @override
...
void loadComponentElement(
  ...
  registerBoxer<ComponentElement>(boxer: (
      {required ComponentElement vmObject,
