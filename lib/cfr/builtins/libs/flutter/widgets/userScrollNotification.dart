...
class VMManagedUserScrollNotification extends _36c2.VMManagedBox<_593c.UserScrollNotification> {VMManagedUserScrollNotification({required this.table, required this.vmObject, required this.hydroState}) : super(
          table: table,
          vmObject: vmObject,
          hydroState: hydroState,
        ) { table  [
'direction'
] = _f376.ScrollDirection.values.indexWhere((x) { return x == vmObject.direction; } );
table  [
'metrics'
...
final _593c.UserScrollNotification vmObject;

 }

class RTManagedUserScrollNotification extends _593c.UserScrollNotification implements _36c2.Box<_593c.UserScrollNotification> {RTManagedUserScrollNotification({required _e2dc.BuildContext context, required _f376.ScrollDirection direction, required _96ae.ScrollMetrics metrics, required this.table, required this.hydroState}) : super(context: context,direction: direction,metrics: metrics) { table  [
'vmObject'
] = vmObject;
table  [
'unwrap'
...
table  [
'direction'
] = _f376.ScrollDirection.values.indexWhere((x) { return x == this.direction; } );
table  [
'metrics'
...
_593c.UserScrollNotification unwrap() => this;
_593c.UserScrollNotification get vmObject => this;
...
void loadUserScrollNotification({required _36c2.HydroState hydroState, required _36c2.HydroTable table}) { table  [
'userScrollNotification'
] = _36c2.makeLuaDartFunc(func: (_fac9.List<_fac9.dynamic> luaCallerArguments) { 


return  [RTManagedUserScrollNotification(table: luaCallerArguments  [
0
...
'context'
] : null, parentState: hydroState), direction: _36c2.maybeUnBoxEnum(values: _f376.ScrollDirection.values, boxedEnum: luaCallerArguments.length >= 2 ? luaCallerArguments  [
1
]  [
'direction'
] : null), metrics: _36c2.maybeUnBoxAndBuildArgument<_96ae.ScrollMetrics, _fac9.dynamic>(luaCallerArguments.length >= 2 ? luaCallerArguments  [
1
]  [
'metrics'
] : null, parentState: hydroState))]; } );
_36c2.registerBoxer<_593c.UserScrollNotification>(boxer: ({required _593c.UserScrollNotification vmObject, required _36c2.HydroState hydroState, required _36c2.HydroTable table}) { return  VMManagedUserScrollNotification(vmObject: vmObject, hydroState: hydroState, table: table); } ); } 
