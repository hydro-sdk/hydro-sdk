...
class VMManagedState extends _36c2.VMManagedBox<_e2dc.State<_e2dc.StatefulWidget>> {VMManagedState({required this.table, required this.vmObject, required this.hydroState}) : super(
          table: table,
          vmObject: vmObject,
...
final _e2dc.State<_e2dc.StatefulWidget> vmObject;

 }

class RTManagedState extends _e2dc.State implements _36c2.Box<_e2dc.State> {RTManagedState({required this.table, required this.hydroState}) : super() { table  [
'vmObject'
] = vmObject;
table  [
'unwrap'
...
_e2dc.State unwrap() => this;
_e2dc.State get vmObject => this;
@_fac9.override _e2dc.StatefulWidget get widget { 
_36c2.Closure closure = table["getWidget"];
return _36c2.maybeUnBoxAndBuildArgument<_e2dc.StatefulWidget, _fac9.dynamic>(closure.dispatch([table],parentState: hydroState)[0], parentState: hydroState); } 
...
void loadState({required _36c2.HydroState hydroState, required _36c2.HydroTable table}) { table  [
'state'
] = _36c2.makeLuaDartFunc(func: (_fac9.List<_fac9.dynamic> luaCallerArguments) { 
return  [RTManagedState(table: luaCallerArguments  [
0
], hydroState: hydroState)]; } );
_36c2.registerBoxer<_e2dc.State>(boxer: ({required _e2dc.State vmObject, required _36c2.HydroState hydroState, required _36c2.HydroTable table}) { return  VMManagedState(vmObject: vmObject, hydroState: hydroState, table: table); } ); } 
