...
class VMManagedStatefulElement extends VMManagedBox<StatefulElement> {
  VMManagedStatefulElement(
    ...
    table['state'] = maybeBoxObject<State>(
        object: vmObject.state, hydroState: hydroState, table: HydroTable());
    ...
    table['update'] = makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
      vmObject.update(maybeUnBoxAndBuildArgument<StatefulWidget, dynamic>(
  ...
  final StatefulElement vmObject;
}

class RTManagedStatefulElement extends StatefulElement
    implements Box<StatefulElement> {
  RTManagedStatefulElement(StatefulWidget widget$,
    ...
    table['state'] = maybeBoxObject<State>(
        object: state, hydroState: hydroState, table: HydroTable());
    table['_dart_build'] =
        makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
      return [
        maybeBoxObject<Widget>(
    ...
    table['_dart_update'] =
        makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
      super.update(maybeUnBoxAndBuildArgument<StatefulWidget, dynamic>(
          luaCallerArguments[1],
          parentState: hydroState));
      return [];
    });
  ...
  StatefulElement unwrap() => this;
  StatefulElement get vmObject => this;
  @override
  Widget build() {
...
void loadStatefulElement(
    {required HydroState hydroState, required HydroTable table}) {
  table['statefulElement'] =
      makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
    return [
      RTManagedStatefulElement(
          maybeUnBoxAndBuildArgument<StatefulWidget, dynamic>(
              luaCallerArguments[1],
              parentState: hydroState),
  ...
  registerBoxer<StatefulElement>(boxer: (
      {required StatefulElement vmObject,
      required HydroState hydroState,
      required HydroTable table}) {
    return VMManagedStatefulElement(
