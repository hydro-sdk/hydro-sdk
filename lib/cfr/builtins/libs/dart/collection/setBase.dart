...
class VMManagedSetBase extends VMManagedBox<SetBase<dynamic>> {
  VMManagedSetBase(
  ...
  final SetBase vmObject;
}

void loadSetBase({required HydroState hydroState, required HydroTable table}) {
  table['setBaseSetToString'] =
      makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
    return [
      SetBase.setToString(maybeUnBoxAndBuildArgument<Set<dynamic>, dynamic>(
          luaCallerArguments[1],
          parentState: hydroState)),
    ];
  });
  registerBoxer<SetBase>(boxer: (
      {required SetBase vmObject,
      required HydroState hydroState,
      required HydroTable table}) {
    return VMManagedSetBase(
