    ...
    table!['sort'] = makeLuaDartFunc(func: (List<dynamic> args) {
      ...
      vmObject!.sort(compare != null
          ? ((a, b) => compare.dispatch(
                [args[0], a, b],
                parentState: hydroState!,
              )![0]) as int Function(Float64x2, Float64x2)?
          : null);
      return [];
    });
            ...
            object: vmObject!.firstWhere(
                ...
                orElse: orElse != null
                    ? (() => maybeUnBoxAndBuildArgument<Float64x2>(
                        ...
                        parentState: hydroState!)) as Float64x2 Function()?
                    : null),
            hydroState: hydroState!,
            table: HydroTable())
      ];
...
void loadFloat64x2List(
  ...
  table['float64x2List'] = makeLuaDartFunc(func: (List<dynamic> args) {
    return [
      maybeBoxObject<Float64x2List>(
          object: Float64x2List(args[1]),
          hydroState: hydroState,
          table: args[0])
    ];
  });
  table['float64x2ListFromList'] = makeLuaDartFunc(func: (List<dynamic> args) {
    return [
      maybeBoxObject<Float64x2List>(
          object: Float64x2List.fromList(
              maybeUnBoxAndBuildArgument<List<Float64x2>>(args[1],
                  parentState: hydroState)),
          hydroState: hydroState,
          table: HydroTable())
    ];
  });
  table['float64x2ListView'] = makeLuaDartFunc(func: (List<dynamic> args) {
    return [
      maybeBoxObject<Float64x2List>(
