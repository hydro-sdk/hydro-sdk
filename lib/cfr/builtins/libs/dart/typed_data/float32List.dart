...
class VMManagedFloat32List extends VMManagedBox<Float32List?> {
  VMManagedFloat32List(
      {required this.table,
    ...
    table!['sort'] = makeLuaDartFunc(func: (List<dynamic> args) {
      ...
      vmObject!.sort(compare != null
          ? ((a, b) => compare.dispatch(
                [args[0], a, b],
                parentState: hydroState!,
              )![0]) as int Function(double, double)?
          : null);
      return [];
    });
        ...
        maybeBoxObject<Iterable>(
            object: vmObject!.expand(f != null
                ? ((element) => maybeUnBoxAndBuildArgument<Iterable<dynamic>>(
                    ...
                    parentState: hydroState!)) as Iterable<dynamic> Function(double)
                : null),
            hydroState: hydroState!,
            table: HydroTable())
      ];
      ...
      return [
        vmObject!.reduce(combine != null
            ? ((value, element) => combine.dispatch(
                  ...
                  parentState: hydroState!,
                )![0]?.toDouble()) as double Function(double, double)
            : null)
      ];
    });
  ...
  final Float32List? vmObject;
}

void loadFloat32List(
    {required HydroState hydroState, required HydroTable table}) {
  table['float32List'] = makeLuaDartFunc(func: (List<dynamic> args) {
    return [
      maybeBoxObject<Float32List>(
          object: Float32List(args[1]), hydroState: hydroState, table: args[0])
    ];
  });
  table['float32ListFromList'] = makeLuaDartFunc(func: (List<dynamic> args) {
    return [
      maybeBoxObject<Float32List>(
          object: Float32List.fromList(maybeUnBoxAndBuildArgument<List<double>>(
              args[1],
              parentState: hydroState)),
          hydroState: hydroState,
          table: HydroTable())
  ...
  table['float32ListView'] = makeLuaDartFunc(func: (List<dynamic> args) {
    return [
      maybeBoxObject<Float32List>(
          object: Float32List.view(
  ...
  table['float32ListSublistView'] = makeLuaDartFunc(func: (List<dynamic> args) {
    return [
      maybeBoxObject<Float32List>(
          object: Float32List.sublistView(
  ...
  registerBoxer<Float32List>(boxer: (
      {required Float32List? vmObject,
      required HydroState? hydroState,
      required HydroTable? table}) {
    return VMManagedFloat32List(
