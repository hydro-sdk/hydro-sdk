...
class VMManagedList extends VMManagedBox<List<dynamic>> {
  VMManagedList(
    ...
    table['insertAll'] =
      ...
      vmObject.insertAll(
          luaCallerArguments[1],
          maybeUnBoxAndBuildArgument<Iterable<dynamic>>(luaCallerArguments[2],
              parentState: hydroState));
      return [];
    });
    ...
    table['setRange'] =
      ...
      vmObject.setRange(
          luaCallerArguments[1],
          luaCallerArguments[2],
          maybeUnBoxAndBuildArgument<Iterable<dynamic>>(luaCallerArguments[3],
              parentState: hydroState),
...
void loadList({required HydroState hydroState, required HydroTable table}) {
  table['list'] = makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
    return [
      maybeBoxObject<List<dynamic>>(
          object: List(luaCallerArguments[1]),
          hydroState: hydroState,
  ...
  table['listCopyRange'] =
    ...
    List.copyRange(
        maybeUnBoxAndBuildArgument<List<dynamic>>(luaCallerArguments[1],
            parentState: hydroState),
        luaCallerArguments[2],
        maybeUnBoxAndBuildArgument<List<dynamic>>(luaCallerArguments[3],
            parentState: hydroState),
        luaCallerArguments[4],
        luaCallerArguments[5]);
    return [];
  });
  table['listWriteIterable'] =
      makeLuaDartFunc(func: (List<dynamic> luaCallerArguments) {
    List.writeIterable(
        maybeUnBoxAndBuildArgument<List<dynamic>>(luaCallerArguments[1],
            parentState: hydroState),
